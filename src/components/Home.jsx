...
const Home = () => {
  ...
  const handleSimpleModeItemClick = (item) => {
    ...
    // Auto-scroll to initial response section
    const scrollToInitialResponse = () => {
      const initialResponseSection = document.getElementById('initial-response-section');
      if (initialResponseSection) {
        initialResponseSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else {
        // Fallback: scroll modal body to top if section not found
        const modalBody = document.querySelector('.modal-body');
        if (modalBody) {
          modalBody.scrollTop = 0;
        }
    ...
    // Scroll after modal is rendered
    setTimeout(scrollToInitialResponse, 100);
    setTimeout(scrollToInitialResponse, 300);
    setTimeout(scrollToInitialResponse, 500);
  };
                ...
                <div className="objection-item">
                  ...
                  <div className="rebuttal-content">
                    <div className="modal-tags">
                      {selectedSimpleModeItem.tags?.map(tag => (
                  ...
                  <div className="modal-rebuttal-content">
                    ...
                    {selectedSimpleModeItem.content && (
                      <>
                        <div className="rebuttal-section" id="initial-response-section">
                    ...
                    <div className="situations-list">
                      <div className="situation-item">
                        ...
                        <div className="situation-content">
                          <h4 className="situation-title">Initial Contact</h4>
                          <p className="situation-description">When first addressing the customer's concern about pricing or budget constraints</p>
                        </div>
                      </div>
                      <div className="situation-item">
                        <div className="situation-icon">💬</div>
                        <div className="situation-content">
                          <h4 className="situation-title">Follow-up Discussion</h4>
                          <p className="situation-description">When customer needs more information about payment options or financing</p>
                        </div>
                      </div>
                      <div className="situation-item">
                        <div className="situation-icon">🤝</div>
                        <div className="situation-content">
                          <h4 className="situation-title">Negotiation Phase</h4>
                          <p className="situation-description">During price discussions or when presenting alternative solutions</p>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        const unsubscribe = onSnapshot(rebuttalsQuery, (snapshot) => {
          ...
          const transformedCategories = fetchedCategories.map(category => {
            ...
            const categoryRebuttals = rebuttals.filter(rebuttal => {
              // Check if rebuttal's category matches the category name OR category ID
              // Also handle cases where category might be stored differently
              const rebuttalCategory = rebuttal.category || '';
              const categoryName = category.name || '';
              const categoryId = category.id || '';
              
              return rebuttalCategory === categoryName || 
                     rebuttalCategory === categoryId ||
                     rebuttalCategory.toLowerCase() === categoryName.toLowerCase();
            });

            console.log(`Category ${category.name} (ID: ${category.id}) has ${categoryRebuttals.length} rebuttals`);
            if (categoryRebuttals.length > 0) {
              console.log(`  Rebuttals in ${category.name}:`, categoryRebuttals.map(r => ({ id: r.id, title: r.title, category: r.category })));
            }
    ...
    if (!selectedCategory) {
      return (
        <div className="no-categories">
          ...
          <p>Please select a category from the list to view its contents</p>
        </div>
      );
    }
