  ...
  async loginUser(email, password) {
    try {
      ...
      if (!registrationSnapshot.empty) {
        ...
        if (status === 'pending') {
          throw new Error('REGISTRATION_PENDING: Your account is still being reviewed. Please wait to be contacted, or contact us at contact@mellowsites.com for assistance.');
        }

        if (status === 'rejected') {
          throw new Error('REGISTRATION_REJECTED: Your registration request has been rejected. Please contact contact@mellowsites.com for more information.');
        }
