...
class RebuttalsService {
  ...
  // Check for document ID mismatches and refresh data
  async checkAndFixDocumentMismatches() {
    try {
      console.log('Checking for document ID mismatches...');
      
      // Get all current rebuttals from Firestore
      const rebuttalsQuery = query(this.rebuttalsCollection, orderBy('createdAt', 'desc'));
      const querySnapshot = await getDocs(rebuttalsQuery);
      const currentRebuttals = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
      
      console.log(`Found ${currentRebuttals.length} rebuttals in Firestore`);
      
      // Log all document IDs for debugging
      const documentIds = currentRebuttals.map(r => r.id);
      console.log('Available document IDs:', documentIds);
      
      return {
        totalRebuttals: currentRebuttals.length,
        documentIds: documentIds,
        rebuttals: currentRebuttals
      };
    } catch (error) {
      console.error('Error checking document mismatches:', error);
      throw error;
    }
  }
  ...
  async updateRebuttal(id, rebuttalData) {
    try {
      ...
      const rebuttalDoc = await getDoc(rebuttalRef);
      
      if (!rebuttalDoc.exists()) {
        // Check if it might be in the archived collection
        const archivedRef = doc(this.archivedCollection, id);
        const archivedDoc = await getDoc(archivedRef);
        
        if (archivedDoc.exists()) {
          throw new Error(`Rebuttal with ID ${id} has been archived and cannot be updated. Please unarchive it first.`);
        } else {
          throw new Error(`Rebuttal with ID ${id} does not exist. It may have been deleted or the ID is incorrect.`);
        }
      }
